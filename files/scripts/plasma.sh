...
URL=$(curl -sL "$API_URL" | grep '"browser_download_url":' | grep -o 'https://[^"]*')

if [[ -z "$URL" ]]; then
  echo "Error: could not determine release url." >&2
  exit 1
fi
...
echo "Extracting contents to $DEST"
tar -xvf "$TMPDIR/release.tar.gz" -C "$DEST"
